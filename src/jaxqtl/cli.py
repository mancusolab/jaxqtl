...
#     base = f"jaxQTL {args[0]}{os.linesep}"
#     rest = args[1:]
#     rest_strs = []
#     needs_tab = True
#     for cmd in rest:
...
#     """
#     # # load inputs
#     # log.info(f'Reading phenotypes ({args.phenotype_bed})')
...
#     # assert phenotype_pos_df.columns[1] == 'pos', \
...
#     #     log.info(f'  * reading covariates ({args.covariates})')
...
#     #         f"Paired covariate phenotypes must be present in phenotype matrix."
#     #     assert paired_covariate_df.columns.equals(phenotype_df.columns), \
#     #         f"Paired covariate samples must match samples in phenotype matrix."
#     # else:
#     #     paired_covariate_df = None
#     #
#     # if args.interaction is not None:
#     #     log.info(f'  * reading interaction term(s) ({args.interaction})')
...
#     #     log.info(f'  * loading genotype dosages' if args.dosages else f'  * loading genotypes')
#     #     genotype_df, variant_df = genotypeio.load_genotypes(args.genotype_path,
...
#     #     if args.mode == 'cis':
#     #         res_df = cis.map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df,
...
#     #             cis.map_nominal(genotype_df, variant_df, phenotype_df, phenotype_pos_df, args.prefix,
#     #                             covariates_df=covariates_df,
...
#     #                             output_dir=args.output_dir, write_top=True, write_stats=not args.best_only,
...
#     #                 signif_df.to_parquet(os.path.join(args.output_dir,
#     #                                                   f'{args.prefix}.cis_qtl.signif_pairs.parquet'))
#     #
...
#     #                                          interaction_df=interaction_df,
#     #                                          maf_threshold_interaction=args.maf_threshold_interaction,
#     #                                          group_s=None, window=args.window,
#     #                                          maf_threshold=maf_threshold, run_eigenmt=True,
#     #                                          output_dir=args.output_dir,
#     #                                          write_top=True, write_stats=not args.best_only,
#     #                                          logger=logger, verbose=True)
...
#     #         summary_df.rename(columns={'minor_allele_samples':'ma_samples', 'minor_allele_count':'ma_count'},
#     #                           inplace=True)
#     #         res_df = cis.map_independent(genotype_df, variant_df, summary_df, phenotype_df, phenotype_pos_df,
#     #                                      covariates_df,
#     #                                      group_s=group_s, fdr=args.fdr,
#     #                                      nperm=args.permutations, window=args.window,
#     #                                      maf_threshold=maf_threshold, logger=logger,
...
#     #         summary_df, res = susie.map(genotype_df, variant_df, phenotype_df.loc[ix],
#     #                                     phenotype_pos_df.loc[ix],
#     #                                     covariates_df, paired_covariate_df=paired_covariate_df,
#     #                                     maf_threshold=maf_threshold,
...
#     #         pval_threshold = 1e-5
#     #         log.info(f'  * p-value threshold: {pval_threshold:.2g}')
#     #
#     #     if interaction_df is not None:
...
#     #
#     #     pairs_df = trans.map_trans(genotype_df, phenotype_df, covariates_df=covariates_df,
...
#     #         log.info('  * filtering out cis-QTLs (within +/-5Mb)')
...
#
#
# def create_run_format_parser(subparser):
#     subparser.add_argument("genotype_path", help="Genotypes in PLINK format")
#     subparser.add_argument("phenotype_bed", help="Phenotypes in BED format")
#     subparser.add_argument("prefix", help="Prefix for output file names")
...
#
#
# def create_run_mapping_parser(subparser):
#     subparser.add_argument("genotype_path", help="Genotypes in PLINK format")
#     subparser.add_argument("phenotype_bed", help="Phenotypes in BED format")
#     subparser.add_argument("prefix", help="Prefix for output file names")
#     subparser.add_argument(
...
#         "--paired_covariate",
#         default=None,
#         help="Single phenotype-specific covariate. Tab-delimited file, phenotypes x samples",
#     )
#     subparser.add_argument(
#         "--permutations",
#         type=int,
...
#     )
#     subparser.add_argument("--fdr", default=0.05, type=np.float64, help="FDR for cis-QTLs")
#     subparser.add_argument(
#         "--qvalue_lambda",
#         default=None,
#         type=np.float64,
...
#     )
#     subparser.add_argument("--seed", default=None, type=int, help="Seed for permutations.")
#     subparser.add_argument("-o", "--output_prefix", default=".", help="Output directory")
#
#     return subparser
#
#
...
#         )
#         run_format_parser = create_run_format_parser(subp)
#         run_format_parser.set_defaults(func=run_format)
#
#     elif cmd_str[1] == "mapping":
#         subp = argp.add_subparsers(
#             description="run mapping (cis or trans)",
#             help="Subcommands: format input or run QTL mapping"
#         )
#         run_format_parser = create_run_mapping_parser(subp)
#         run_format_parser.set_defaults(func=run_mapping)
#
