...
| EQ-6 | `log(mu)=o+alpha_i^donor+X_nuis beta` | NB2 null mean with absorbed donor effects | absorbed cell-NB inference | `cis-state.AC2.1` | planned |
| EQ-7 | `X_0^T W X_0 beta = X_0^T W z_0` | Exact weighted FWL reduced solve | grouped absorbed WLS | `cis-state.AC2.2` | planned |
| EQ-8 | `w=mu/(1+phi*mu)` | NB2 log-link IRLS weights | existing family plus absorbed IRLS | `cis-state.AC2.3` | planned |
| EQ-9 | `q_score=(Y-mu)/(1+phi*mu)` | Mean score used by the omnibus test | absorbed-NB result | `cis-state.AC2.4` | planned |
| EQ-10 | `A_score[i,:]=sum_{c in i} q_score[c]S[c,:]` | Donor-state score aggregation | omnibus interaction score | `cis-state.AC3.2` | planned |
| EQ-11 | `U=G^T A_score`, `Q=||U||_F^2` | Standardized-genotype omnibus statistic | omnibus interaction score | `cis-state.AC3.3` | planned |
| EQ-12 | `t_i=u_i-J_HX J_XX^{-1}b_i` | Nuisance-efficient donor contribution | efficient cluster-score operator | `cis-state.AC3.5` | planned |
| EQ-13 | `U_b*=sum_i xi_bi t_i` | Blocked donor multiplier score | multiplier calibration | `cis-state.AC4.1` | planned |
| EQ-14 | `p=(1+sum_b I(Q_b*>=Q))/(B_boot+1)` | Finite-replicate Monte Carlo p-value | multiplier calibration | `cis-state.AC4.2` | planned |
| EQ-15 | `B_cluster=T T^T` | Matrix-free donor-space covariance action | moment calibration operator | `cis-state.AC4.4` | planned |
| EQ-16 | `c_l=tr(B_cluster^l)` | Exact/probed spectral moments with MC error | moment calibration | `cis-state.AC4.5` | planned |
