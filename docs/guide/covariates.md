...
Covariate tables require at least one data column. The reader recognizes `NA`, an empty field, `NULL`, `NaN`,
and `nan` as missing values. An optional `fid` column is ignored. jaxQTL aligns every input on the intersection of sample IDs and preserves the
...
[compute them separately for each cell type](compute-pcs.md).
...
Gene (`--gene-list`), keep-sample (`--keep`), and exclude-sample (`--exclude`) files contain one identifier per line.
A first line beginning with `#` is treated as a header and skipped. Sample lists use IIDs; gene lists use phenotype
IDs from the phenotype matrix.
